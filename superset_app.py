...
if chosen_menu =="1. Задача и актуальность":
    ...
    dop = st.checkbox('Дополнительная информация')
    if dop:
        st.write("""
        В запросе `WHERE` можно комбинировать несколько параметров при помощи оператора `AND`. Так, например, чтобы вывести автомобили марки `LADA` выставленных 
        на продажу в Ленинградской области, нам нужно написать запрос `WHERE Название LIKE '%LADA%' AND iso LIKE 'RU-LEN'`. 
        \n *iso - стандарт, определяющий геокоды для основных единиц первого и второго уровня административно-территориального деления.* [Подробнее](https://ru.wikipedia.org/wiki/ISO_3166-2)
        """)
